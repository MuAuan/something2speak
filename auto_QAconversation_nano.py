# coding: utf-8
#>python auto_QAconversation_.py -d /usr/lib/aarch64-linux-gnu/mecab/dic/mecab-ipadic-neologd conversation_.csv  -s stop_words.txt
...
def save_questions(line):
    with open('conversation_newu.txt', 'a', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
...
def conversation_QA(questions,answers,vecs,mecab,line):
    ...
    line = input("> ")
    with open('conversation_u.csv', 'a', newline='', encoding='utf-8') as f: #a+ #w
        writer = csv.writer(f)
        while True:
            writer.writerow({line})
...
def conversation(questions,vecs,mecab,line):
    print(line+"しましょう(⋈◍＞◡＜◍)。✧♡")
    line = input("> ")
    with open('conversation_u.txt', 'a', newline='', encoding='utf-8') as f: #a+ #w
        writer = csv.writer(f)
        while True:
            writer.writerow({line})
            sims = cosine_similarity(vectorizer.transform([mecab.parse(line)]), vecs)
...
def train_conv(mecab):
    questions = []
    with open('conversation_u.txt', encoding='utf-8') as f:
...
def read_conv(mecab):
    conv_new = []
    with open('conversation_newu.txt', encoding='utf-8') as f:
        cols = f.read().strip().split('\n')
        for i in range(len(cols)):
